...
TODO :
  - TIMEOUTS
  - Cr�er une fonction qui s'occupe de la gestion des timeout + erreurs de transmission
  - Congestion
*/
#include <stdio.h>
#include <stdlib.h>
...
/*
  DEUX REGLEs SUR LE FICHIER A envoyer
	...
	- PAS DE LIGNE � moins de 2 caract�res
*/

int get_numSequence(char* buffer) {
	//R�cup�ration num s�quence
...
void* replace_str(char* str, const char* orig, const char* rep)
{
	...
	//Utilis� pour formater ligne � envoyer
	char* p;
...
int main(int argc, char* argv[])
{
	...
	//D�finition adresse UDP de connexion :
	...
	adresseUDP.sin_port = htons(atoi(argv[1])); //Port du serveur, converti en valeur r�seau
	adresseUDP.sin_addr.s_addr = htonl(INADDR_ANY);

	//D�finition socket UDP
	int socketServUDP;
	...
	adresseUDP_data.sin_port = htons(atoi(argv[2])); //Port du serveur, converti en valeur r�seau
	...
	{
		//TODO : SELECT
		FD_SET(socketServUDP, &socket_set); //Activation du bit associ� � au socket UDP de CONNEXION
		FD_SET(socketServUDP_data, &socket_set); //Activation du bit associ� � au socket UDP de DATA
		...
		if (isInit) {
			...
			if (strcmp(buffer, "") != 0) printf("Re�u UDP : %s\n", buffer);
			...
			if (strstr(buffer, " SYN") != NULL) {
				...
				if (selret != 0) {
					...
					if (strstr(buffer, " ACK")) {
						...
						numSequence++;
						memset(buffer, 0, sizeof(buffer));
						sprintf(buffer, "%i %i", numSequence, atoi(argv[2])); //Envoi du port de donn�es
						sendto(socketServUDP, buffer, strlen(buffer), 0, (const struct sockaddr*) & adresse_arrivee, taille_arrivee);
					}
				}
			}
			...
			if (strstr(buffer, " BEGIN") != NULL) {
				...
				while (fgets(ligne, BUFFER_TAILLE - 7, fichier) != NULL) {
					//Traitement de la ligne : pour le bien de l'envoi, les espaces du message doivent �tre convertis en underscore
