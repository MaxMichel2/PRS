    ...
    while(1)
    {
        ...
        if(args->thread_multi_ack_size != -1)
        {
            ...
            if(ack_received[last_ack-1] == 1){//si l'on reçoit un ack antérieur, il y a eu un drop
              ...
              if(args->thread_duplicate_ack >= 3){
...
int main(int argc, char* argv[])
{
    ...
    while(main_loop)
    {
        ...
        if(multi == 0)
        {
            ...
            thread_args->thread_ssthresh = ssthresh;
            thread_args->thread_msg_drop = msg_drop;
            thread_args->thread_duplicate_ack = duplicate_ack;
