...
#define MAX_ARRAY_SIZE 500 //The max size of a chunk of data to be read and stored in buffer before sending it
...
int split_processus(int s, char buf[BUFLEN], char message[BUFLEN], struct sockaddr_in si_other, int slen, int port, float RTT){
	...
	if (!pid){
		...
		read_timeout.tv_usec = (int)(0.005 * RTT * pow(10,9)); //Calculated RTT
